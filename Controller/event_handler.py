...
def handle_events(event, game_state):
    ...
    if event.type == pygame.QUIT:
    ...
    elif event.type == pygame.KEYDOWN:        # Gérer le menu Options en premier
        if game_state.get('options_menu_active', False):
            from Controller.gui import handle_options_menu_event
            if handle_options_menu_event(event, game_state):
                return
        ...
        # Gérer le menu Options en premier
        if game_state.get('options_menu_active', False):
            from Controller.gui import handle_options_menu_event
            handle_options_menu_event(event, game_state)
            return
            ...
            for label, rect in button_rects.items():
                if rect.collidepoint(mx, my):
                    if label == "Resume":
                        ...
                        game_state['options_menu_active'] = True
